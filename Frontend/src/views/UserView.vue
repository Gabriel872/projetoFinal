        ...
        <div class="flex flex-header">
          <div class="flex perfil">
            <svg
              ...
              height="48"
              fill="currentColor"
              class="bi bi-person-fill avatar"
              viewBox="0 0 16 16"
            >
              <path
      ...
      <div class="row">
        ...
        <div class="col-10">
          <div v-if="option == 0" class="container-fluid" style="">
            ...
            <div class="row">
              ...
              <div class="mb-3">
                ...
                >
                <textarea
                  v-model="obj_user.user_description"
                  class="form-control"
          ...
          <div v-if="option == 1" class="container-fluid" style="">
            ...
            <div class="row">
              <div class="mb-3">
                <label for="passwordFormControlInput1" class="form-label"
                  >Senha</label
                >
                <input
                  v-model="obj_user.user_password"
                  type="text"
                  class="form-control"
              ...
              <div class="mb-3">
                <label for="emailFormControlnput1" class="form-label"
                  >Email</label
                >
                <input
                  v-model="obj_user.user_email"
                  type="email"
                  class="form-control"
                  id="emailFormControlnput1"
...
export default {
  ...
  methods: {
    ...
    async deleteAccount() {
      const request = await fetch(`https://localhost:7114/api/UserCourse`);
      const request_return = await request.json();
      this.users_courses = request_return;

      for (var i = 0; i < this.users_courses.length; i++) {
        if (this.users_courses[i].id_user == this.id_user) {
          await fetch(
            `https://localhost:7114/api/UserCourse/${this.users_courses[i].id_user_course}`,
            {
              method: "DELETE",
              headers: {
      ...
      await fetch(`https://localhost:7114/api/User/${this.id_user}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
    ...
    async updateUser() {
      if (
        this.obj_user.user_password == undefined &&
        this.obj_user.user_email == undefined
      ) {
      ...
      localStorage.setItem("userName", this.obj_user.user_name);
      localStorage.setItem("userEmail", this.obj_user.user_email);
      localStorage.setItem("userDescription", this.obj_user.user_description);

      this.user_name = localStorage.getItem("userName");
      this.user_email = localStorage.getItem("userEmail");
      this.user_description = localStorage.getItem("userDescription");
    },
  },
};
</script>
