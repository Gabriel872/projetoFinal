...
CREATE TABLE courses (
       ...
       course_subtitle VARCHAR2(140) NOT NULL, 
       course_people_amt NUMBER,
       course_rating NUMBER,
       course_language VARCHAR2(30) NOT NULL,
       course_creation_date DATE NOT NULL,
       course_description VARCHAR2(500) NOT NULL,
       course_requirements VARCHAR2(200) NOT NULL,
       course_time NUMBER NOT NULL,
       course_link VARCHAR2(200) NOT NULL,
       course_audience VARCHAR2(100) NOT NULL,
       course_learnings VARCHAR2(160),
       course_knowledge_level VARCHAR2(30) NOT NULL,
       course_message VARCHAR2(100),
       id_author NUMBER, -- tabela usuarios
       id_category NUMBER, -- tabela categorie
...
CREATE TABLE categories (
       id_category NUMBER CONSTRAINT pk_id_category PRIMARY KEY,
...
CREATE OR REPLACE TRIGGER increment_id_category
BEFORE INSERT ON categories
FOR EACH ROW
BEGIN
  :new.id_category := auto_increment_categories.NEXTVAL;
END;
...
CREATE TABLE interests (
       ...
       id_user NUMBER, -- tabela usuarios
       id_category NUMBER -- tabela categories
);
...
CREATE TABLE subcategories (
       id_subcategory NUMBER CONSTRAINT pk_id_subcategory PRIMARY KEY,
...
CREATE OR REPLACE TRIGGER increment_id_subcategory
BEFORE INSERT ON subcategories
FOR EACH ROW
BEGIN
  :new.id_subcategory := auto_increment_subcategories.NEXTVAL;
END;
...
CREATE TABLE sub_themes (
       ...
       id_subcategory NUMBER -- tabela subcategories
);
...
AS
    qtd_courses NUMBER;
BEGIN
   SELECT COUNT(*) INTO qtd_courses FROM courses WHERE id_category = id_categ;
...
SELECT courses.id_category FROM courses INNER JOIN categories ON categories.id_category = courses.id_category WHERE courses.id_category = categories.id_category;
...
INNER JOIN categories
ON courses.id_categorie = categories.id_categorie
WHERE courses.course_name LIKE '%Java%'
